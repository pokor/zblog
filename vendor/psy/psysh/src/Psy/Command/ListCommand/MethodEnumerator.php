...
class MethodEnumerator extends Enumerator
{
    ...
    {
        // only user methods when a Reflector is present.

        if ($reflector === null) {
            return;
        }

        // We can only user methods on actual class (or object) reflectors.
        if (!$reflector instanceof \ReflectionClass) {
            return;
        }

        // only user methods if we are specifically asked
