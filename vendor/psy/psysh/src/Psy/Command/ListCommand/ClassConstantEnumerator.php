...
class ClassConstantEnumerator extends Enumerator
{
    ...
    {
        // only user constants when a Reflector is present.

        if ($reflector === null) {
            return;
        }

        // We can only user constants on actual class (or object) reflectors.
        if (!$reflector instanceof \ReflectionClass) {
            // TODO: handle ReflectionExtension as well
            return;
        }

        // only user constants if we are specifically asked
