...
class PropertyEnumerator extends Enumerator
{
    ...
    {
        // only user properties when a Reflector is present.

        if ($reflector === null) {
            return;
        }

        // We can only user properties on actual class (or object) reflectors.
        if (!$reflector instanceof \ReflectionClass) {
            return;
        }

        // only user properties if we are specifically asked
